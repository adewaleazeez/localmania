...
export default class VoucherAuditDocs extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  getBase64() {
    ...
    reader.onload = (event) => {
        ...
        this.callCreateVoucherAuditDoc();
    };
  ...
  callCreateVoucherAuditDoc = async() => {
    ...
    if(this.state.base64File=="" || this.state.base64File==undefined || this.state.base64File==null){
      ...
      return true;
    }//id, description, voucherAuditId, scannedDoc*/
    ...
    await UsersDataService.createVoucherAuditDoc(data)
    .then(response => {
  ...
  callDeleteVoucherAuditDoc = async(record_id) => {
    this.setState({ loading: true });
    var data = {
      id: record_id,
    };
    await UsersDataService.deleteVoucherAuditDoc(data)
      .then(response => {
  ...
  getVoucherAuditDocs = async() => {
    var data = {
      voucherAuditId: localStorage.getItem('voucherid'),
    };
    await UsersDataService.getVoucherAuditDocs(data)
      .then(res => {
  ...
  render() {
    ...
    let sno = 0;
    let voucherAuditDocs = items.map(item => {
      return (
        <div className="nk-tb-item" key={item.id}>
          ...
          <div className="nk-tb-col tb-col-md">
            <span className="tb-actions">
              ...
              <a 
                href="#"
                onClick={() => this.callDeleteVoucherAuditDoc(item.id)}
              >
                Delete
              </a>
