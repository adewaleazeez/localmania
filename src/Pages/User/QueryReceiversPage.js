...
export default class QueryReceiversPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  componentWillMount(){
    if(localStorage.getItem('unitid') != 10){
      alert("Access Denid!!!\nOnly Admin is allowed access to Query Receivers Setup menu");
      ...
      <form className="form-validate is-alter">
        <div className="form-group">
          ...
          <div className="form-control-wrap">
            ...
            <input
              type="text"
              id="receiverNameEdit"
              name="receiverNameEdit"
              ref="receiverNameEdit"
              className="form-control"
              required
        ...
        <div className="form-group">
          {this.state.selectedRecord.title == "Edit" ?
            <button 
              onClick={() => this.callUpdateQueryReceiver()}
              type="button" 
              className="btn btn-lg btn-primary"
            >
              Update Query Receiver
            </button>
            :
            <button 
              onClick={() => this.callDeleteQueryReceiver()}
              type="button" 
  ...
  newForm() {
    return (
      <form className="form-validate is-alter">
        <div className="form-group">
          <label className="form-label">Query Receiver Name</label>
          <div className="form-control-wrap">
            <input 
              type="text" 
              id="receiverName"
        ...
        <div className="form-group">
          <button 
            onClick={() => this.callCreateQueryReceiver()}
            type="button" 
            className="btn btn-lg btn-primary"
          >
            Create Query Receiver
          </button>
        </div>
      </form>
    );
  ...
  callCreateQueryReceiver = async() => {
    this.setState({ loading: true });
    if(this.refs.receiverName.value==""){
      Toastr(
        "error",
        "Invalid Query Receiver Name"
      );
      return true;
    }
    var data = {
    ...
    await UsersDataService.createQueryReceiver(data)
      .then(response => {
        ...
        if(response.toString().includes("already exists")){
          ...
          Toastr(
            "info",
            "Query Receiver creation is successful...."
          );
        }
  ...
  callUpdateQueryReceiver = async() => {
    this.setState({ loading: true });
    if(this.refs.receiverNameEdit.value==""){
      Toastr(
        "error",
        "Invalid Query Receiver Name"
      );
    ...
    var data = {
      ...
      receiverName: this.refs.receiverNameEdit.value,
      description: this.refs.descriptionEdit.value
    };
    await UsersDataService.updateQueryReceiver(data)
      .then(response => {
        ...
        if(response.toString().includes("does not exist")){
          ...
          Toastr(
            "info",
            "Query Receiver update is successful...."
          );
        }
        this.setState({ modalOpen: false});
        this.getQueryReceivers();
  ...
  callDeleteQueryReceiver = async() => {
    ...
    await UsersDataService.deleteQueryReceiver(data)
      .then(response => {
        if(typeof(response) !== "object"){
          ...
          Toastr(
            "info",
            "Query Receiver delete is successful...."
          );
        }
        this.setState({ modalOpen: false});
        this.getQueryReceivers();
      ...
      .catch(e => {
        Toastr(
          "error",
          "Error deleting query receiver "+e
        );
        this.setState({ loading: false });
      });
  ...
  handleKeyPress = (e) => {
      var key = e.key;
      if (key == "Enter") {
        this.getQueryReceiversByOption();
      }
      
  };

  getQueryReceiversByOption = async() => {
    this.setState({ loading: true });
    ...
    await UsersDataService.getQueryReceiversByOption(data)
      .then(res => {
        const {perPage} = this.state;
        const list = res.data;
        this.setState({
  ...
  render() {
    ...
    let receivers = items.map(item => {
      return (
        <div className="nk-tb-item" key={item.id}>
          <div className="nk-tb-col tb-col-md">
            <span className="tb-queryreceiver">{item.receiverName}</span>
          </div>
            ...
            <div className="toggle-expand-content" data-content="pageMenu">
              <ul className="nk-block-tools g-3">
                ...
                <li className="nk-block-tools-opt">
                  ...
                  <button
                    type="button"
                    onClick={() => this.getQueryReceiversByOption()}
                  ...
                  <button
                    type="button"
                    onClick={() => this.getQueryReceivers()}
                    className="toggle btn btn-primary d-none d-md-inline-flex"
                  >
        ...
        <CustomModal
          title={
            this.state.selectedRecord
              ? (this.state.selectedRecord.title == "Edit" ? "Edit Query Receiver" : "Delete Query Receiver")
              : "New Query Receiver"
          }
