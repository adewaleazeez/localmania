...
export default class AdminPasswordChangePage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  getUser(){
    ...
    UsersDataService.findByUserName(localStorage.getItem('username'))
      ...
      .catch(e => {
        ...
        Toastr(
          "error",
          "There is problem changing admin's password"
        );
  ...
  UpdateUser() {
    ...
    UsersDataService.passwordChange(id, data)
      .then(response => {
        Toastr(
          "info",
          "Admin password change is successful...., check your email box for the new password!!!"
        );
      ...
      .catch(error => {
        ...
        Toastr(
          "error",
          "There is problem changing the admin password, "+error.response.data.message
        );
        this.setState({ loading: false });
      });
